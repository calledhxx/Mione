...
```
如上所示，函數一的語式為主語式，而函數二的語式是函數語式，`...`內容物之語式亦然是函數語式。
...
# 作用域
作用域是指五大類別物件所處的空間，例如：`function... end`，`...`的作用域為該函數。

作用域與語式不同地方在於作用域有無限多個，但語式卻只有主語式、函數語式、表單語式...。
...
```
如上所示，函數一並未有作用域，而函數二的作用域是函數一本身，`...`內容物之作用域是函數二。
...
# 明暗示
明示與暗示是區分「源代碼內的五大類別」與「運算後所產生的五大類別」的特殊用詞。

## 明示
明示是源代碼內五大類別的總稱，其特性為「直接固定型」意旨理論上物件會直接向Mione表示，且其效果及內容是固定的。
...
## 片語樣式
絕對片語的宣告是以 `#` （井字符） 加上絕對片語名稱。例如：`#local`。
