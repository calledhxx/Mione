...
COMPUTATIONRespondObj COMPUTATION(COMPUTATIONRequestObj input)
{
    ...
    for (int OrderOfOperations = 0;;OrderOfOperations++)
    {
        unsigned char LowestRequestedOrder = 255;
            ...
            case SYMBOL:
            {
                ...
                default:
                {
                    if (!BracketsChild)
                    {
                        ...
                        case SYMBOL_ADD:
                        {
                            if (OrderOfOperations < 2)
                            {
                                LowestRequestedOrder = _min(LowestRequestedOrder, 2);
                                continue;
                            }
                            if (OrderOfOperations > 2) continue;
        ...
        if (OrderOfOperations == LowestRequestedOrder) break;
        OrderOfOperations = LowestRequestedOrder - 1;
    }
